...
+ 使用`rosu-memory-lib`库实现了测试中的实时导出功能：可以记录程序启动后游玩的成绩。
...
## v0.2.1
+ 现在只有在开启实时模式才会启动读osu内存的线程，关闭后自动关闭该线程
+ 在实时模式添加卡片完成后调用`request_redraw()`，避免了窗口因为挂在后台而休眠，同时添加了刷新按钮
+ 修复了一些实时模式读取内存的bug
+ 实时模式现在卡片的生成按倒序排序，即最新的永远在最前面
+ 移除了实时模式右下角移除按钮的功能，以避免可能的错误，未来可能考虑添加实时模式卡片转移到普通模式
+ 添加了对Hard Rock和Easy mod的支持
+ 使用新版`rosu-memory-lib`库解决6月以来osu内存的重大变化
+ 修复了marvellous判定在超过一定比例（大概是96.8%）的绘制错误。
+ 添加了选择osu!文件夹的功能
+ ~~目前存在的问题：疑似对使用zzzzv的OsuManiaToolBox处理过的数据库会产生bug，在osu_db库读取时提示Error: failed to parse osu file: Map on Option~~已解决：osu中的Fail Grade(值为8，byte类型)不能存储于osu!.db中，否则`osu-db`库无法parse。