...
use osu_db::{self, Listing, Mod, ModSet, ScoreList};
use rayon::prelude::*;
...
use crate::db::{get_db_path, get_osu_install_path};
...
#[inline]
fn calc_mod_rating(mods: ModSet, srs: (f64, f64, f64), acc: f64) -> (f64, f64) {
...
pub fn prepare_ratings(osu_exe_dir: &str) -> io::Result<HashMap<String, Vec<RatingInfo>>> {
    // 读取谱面数据库
    let (mut all_ratings, mut _best_ratings) = extract_ratings(osu_exe_dir)?;
